...
class UpbitQuoteManagerImplTest {
    ...
    public void makeParams() {
        // GIVEN
        List<String> markets = List.of("KRW-BTC", "KRW-ETH", "KRW-XRP");
        UpbitQuoteManagerImpl quoteManager = new UpbitQuoteManagerImpl();

        // WHEN
        String s = quoteManager.makeUrlParams(markets);

        // THEN
        assertEquals("&markets=KRW-BTC&markets=KRW-ETH&markets=KRW-XRP", s);
    }
}